...
function SPZ.Notify(msg, type, time)
    exports['spz-lib']:Notify(msg, type or 'info', time or 3000)
end
...
AddEventHandler("spz-stance:airsuspension", function(vehicle,val,coords)
...
RegisterNUICallback('setvehicleheight', function(data, cb)
    ...
    if vehicle ~= nil and vehicle ~= 0 and not busyairsus then
		print("HEIGHT")
		busyairsus = true
		TriggerServerEvent("spz-stance:airsuspension",VehToNet(vehicle), data.val, GetEntityCoords(vehicle))
    end
	cb(true)
end)
...
RegisterNetEvent("spz-stance:openstancer")
AddEventHandler("spz-stance:openstancer", function(vehicle,val,coords)
	OpenStancer()
end)
...
RegisterNUICallback('wheelsetting', function(data, cb)
    ...
    if vehicle ~= nil and vehicle ~= 0 then
		...
		SPZ.Notify("Vehicle Wheel Data is Saved", "success")
	end
	cb(true)
end)
...
function OpenStancer()
	...
	if vehicle  ~= 0 and #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(vehicle )) < 15 and GetVehicleDoorLockStatus(vehicle ) == 1 then
		...
		if GetVehicleDoorLockStatus(vehicle ) ~= 1 then
			SPZ.Notify("No Unlock Vehicle Nearby", "error")
		else
			SPZ.Notify("No Nearby Vehicle", "error")
		end
	end
end
